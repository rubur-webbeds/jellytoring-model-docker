...
def predict(input_folder):
    inference_status = 'ko'
    flags = tf.app.flags
    flags.DEFINE_string('inference_graph_dir', 'model', 'Directory where the inference graph is stored.')
    flags.DEFINE_string('print_thr', '0.1', 'Printing confidence threshold.')
    flags.DEFINE_string('test_dir', input_folder, 'test images folder path.')
    flags.DEFINE_string('out_dir', input_folder + '/results', 'output folder path.')
    flags.DEFINE_string('n_classes', '4', 'number of classes.')
    flags.DEFINE_string('labels_file', 'model/object_detection.pbtxt', 'path to the class label file')
            ...
            for root, dirs, files in os.walk(PATH_TO_TEST_IMAGES_DIR):  # for each folder
                ...
                for file in enumerate(files):  # for each file in the folder
                    ...
                    if re.search("\.(png|jpg|jpeg)$", file[1]):  # if the file is an image
                        ...
                        if not os.path.exists(PATH_TO_OUTPUT_DIR + "/txt"):
                            os.makedirs(PATH_TO_OUTPUT_DIR + "/txt")

                        name_out, ext = os.path.splitext(file[1])

                        file_out = os.path.join(PATH_TO_OUTPUT_DIR + "/txt", name_out + ".txt")
                        ...
                        # save image
                        if not os.path.exists(PATH_TO_OUTPUT_DIR + "/output"):
